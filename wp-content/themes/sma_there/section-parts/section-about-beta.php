...
<section id="<?php if ( $id != '' ) { echo esc_attr( $id ); } ?>"
	class="section-about section-padding onepage-section section-about-beta">
	...
	<div class="container">
		...
		if ( is_array( $page_ids ) && ! empty( $page_ids ) ) :
			...
			/* ── 1. Main About Us — Magazine / Koran Layout ── */
			if ( $about_main ) :
				$post     = $about_main['post'];
				$settings = $about_main['settings'];
				?>
				<div class="about-main-newspaper clearfix wow slideInUp" data-wow-delay="0.1s">
				...
				</div><!-- .about-main-newspaper -->
			<?php endif; /* end about_main */

			/* ── 2. Visi & Misi — Side-by-Side Cards ── */
			if ( ! empty( $visi_misi_pages ) ) :
				$vm_icons = array( 'fa-eye', 'fa-rocket', 'fa-star', 'fa-heart' );
				$vm_index = 0;
				?>
				<div class="visi-misi-cards-row row">
					<?php foreach ( $visi_misi_pages as $item ) :
						...
						$icon     = isset( $vm_icons[ $vm_index ] ) ? $vm_icons[ $vm_index ] : 'fa-check-circle';
						$vm_index++;
						?>
						<div class="col-md-6 col-sm-12 visi-misi-card-col wow slideInUp" data-wow-delay="<?php echo ( $vm_index * 0.1 ); ?>s">
							<div class="visi-misi-card">

								<h4 class="visi-misi-card-title">
									<span class="visi-misi-card-icon-badge" aria-hidden="true" style="display:inline-flex;width:36px;height:36px;align-items:center;justify-content:center;background:var(--th-green-100);border-radius:50%;color:var(--th-green-600);font-size:16px;flex-shrink:0;">
										<i class="fa <?php echo esc_attr( $icon ); ?>"></i>
									</span>
									<?php if ( ! $settings['hide_title'] ) echo get_the_title( $post ); ?>
								</h4>
					...
					<?php endforeach; ?>
				</div><!-- .visi-misi-cards-row -->
			<?php endif; /* end visi_misi_pages */
