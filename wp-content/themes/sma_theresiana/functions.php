...
if ( ! function_exists( 'sma_theresiana_widgets_init' ) ) :
    ...
    function sma_theresiana_widgets_init() {
        ...
        // Footer PPDB Widget Area
        register_sidebar( [
            'name'          => __( 'Footer PPDB Image', 'sma-theresiana' ),
            'id'            => 'th-footer-ppdb',
            'description'   => __( 'Tambahkan widget "Image/Gambar" ke sini untuk brosur PPDB.', 'sma-theresiana' ),
...
// Force beta=1 on all URLs generated by WordPress (for preview mode)
function th_append_beta_query( $url ) {
    if ( ! is_admin() && ! empty( $url ) ) {
        // Parse URL to check if it's the current site
        $home_url = home_url();
        if ( strpos( $url, $home_url ) !== false ) {
            $url = add_query_arg( 'beta', '1', $url );
        }
    }
    return $url;
}
add_filter( 'post_link', 'th_append_beta_query', 10, 1 );
add_filter( 'page_link', 'th_append_beta_query', 10, 1 );
add_filter( 'post_type_link', 'th_append_beta_query', 10, 1 );
add_filter( 'category_link', 'th_append_beta_query', 10, 1 );
add_filter( 'term_link', 'th_append_beta_query', 10, 1 );
add_filter( 'author_link', 'th_append_beta_query', 10, 1 );
add_filter( 'day_link', 'th_append_beta_query', 10, 1 );
add_filter( 'month_link', 'th_append_beta_query', 10, 1 );
add_filter( 'year_link', 'th_append_beta_query', 10, 1 );

// Force beta=1 on menu item links
add_filter( 'nav_menu_link_attributes', function( $atts, $item, $args ) {
    if ( ! is_admin() && ! empty( $atts['href'] ) ) {
        if ( strpos( $atts['href'], '#' ) !== 0 ) {
            $atts['href'] = add_query_arg( 'beta', '1', $atts['href'] );
        }
    }
    return $atts;
}, 10, 3 );
