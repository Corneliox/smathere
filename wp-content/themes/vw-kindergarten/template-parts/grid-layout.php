...
<div class="col-lg-4 col-md-6">
	<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
	    <div class="post-main-box p-3 mb-3 text-center wow zoomIn" data-wow-duration="2s">
	        ...
	        <?php if( get_theme_mod( 'vw_kindergarten_grid_postdate',true) == 1 || get_theme_mod( 'vw_kindergarten_grid_author',true) == 1 || get_theme_mod( 'vw_kindergarten_grid_comments',true) == 1) { ?>
		        <div class="post-info">
		            ...
		            <?php if(get_theme_mod('vw_kindergarten_grid_postdate',true)==1){ ?>
		                <i class="<?php echo esc_attr(get_theme_mod('vw_kindergarten_grid_postdate_icon','fas fa-calendar-alt')); ?> me-2"></i><span class="entry-date"><a href="<?php echo esc_url( get_day_link( $vw_kindergarten_archive_year, $vw_kindergarten_archive_month, $vw_kindergarten_archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span>
		            <?php } ?>

		            <?php if(get_theme_mod('vw_kindergarten_grid_author',true)==1){ ?>
		                <span><?php echo esc_html(get_theme_mod('vw_kindergarten_single_post_meta_field_separator', '|'));?></span> <i class="<?php echo esc_attr(get_theme_mod('vw_kindergarten_grid_author_icon','fas fa-user')); ?> me-2"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span>
		            <?php } ?>

		            <?php if(get_theme_mod('vw_kindergarten_grid_comments',true)==1){ ?>
		                <span><?php echo esc_html(get_theme_mod('vw_kindergarten_single_post_meta_field_separator', '|'));?></span> <i class="<?php echo esc_attr(get_theme_mod('vw_kindergarten_grid_comments_icon','fa fa-comments')); ?> me-2" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'vw-kindergarten'), __('0 Comments', 'vw-kindergarten'), __('% Comments', 'vw-kindergarten') ); ?></span>
		            <?php } ?>
		            <hr>
		        </div>
		    <?php } ?>
	        <div class="new-text">
	        	<p>
			        <?php $vw_kindergarten_excerpt = get_the_excerpt(); echo esc_html( vw_kindergarten_string_limit_words( $vw_kindergarten_excerpt, esc_attr(get_theme_mod('vw_kindergarten_related_posts_excerpt_number','30')))); ?> <?php echo esc_html( get_theme_mod('vw_kindergarten_excerpt_suffix','') ); ?>
	        	</p>
	        </div>
